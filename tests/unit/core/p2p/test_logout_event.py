...
def test_stream_orchestrator_tears_down_and_can_be_recreated(nc_loop) -> None:
    ...
    with (
        ...
        assert manager._logout_listener not in auth.listeners(AUTH_LOGOUT_EVENT)
        assert replacement._logout_listener in auth.listeners(AUTH_LOGOUT_EVENT)

        nc_loop.call_soon_threadsafe(replacement.close)
        assert replacement_session.closed_event.wait(timeout=2)
        assert auth.listeners(AUTH_LOGOUT_EVENT) == []
