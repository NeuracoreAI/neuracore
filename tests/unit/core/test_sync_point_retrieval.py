...
def test_sync_point_by_name_returns_connected_robot_data(
    temp_config_dir,
    mock_auth_requests,
    reset_neuracore,
    mocked_org_id,
):
    """Return the connected robot's data when requested by name."""
    nc.login("test_api_key")
    mock_auth_requests.post(
        re.compile(f"{API_URL}/org/[^/]+/robots(\\?.*)?"),
        json=_register_robot_with_unique_id,
    ...
    sync_point = nc.get_latest_sync_point(robot_name="robot-a", include_remote=False)

    assert set(sync_point.data[DataType.JOINT_POSITIONS]) == {"joint_on_a"}


def test_sync_point_for_unregistered_robot_raises_robot_error(
    temp_config_dir,
    mock_auth_requests,
    reset_neuracore,
    mocked_org_id,
):
    """Raise RobotError when the requested robot name is not registered."""
    nc.login("test_api_key")
    mock_auth_requests.post(
        re.compile(f"{API_URL}/org/[^/]+/robots(\\?.*)?"),
        json=_register_robot_with_unique_id,
    ...
    with pytest.raises(RobotError, match="not initialized"):
        nc.get_latest_sync_point(robot_name="robot-b", include_remote=False)
