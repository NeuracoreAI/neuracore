...
# Spelled out here rather than imported from the SDK: these tests must fail
# if the SDK's outbound value ever drifts from what the backend accepts.
BACKWARD_WIRE_VALUE = "BACKWARD"
...
def _make_paginated_post(all_recordings: list[dict], total: int | None = None):
    ...
    and the number of recordings the response carried.
    """
    ...
    def fake_post(url, headers=None, params=None, json=None, timeout=None):
        ...
        record = {"params": params, "json": json}
        calls.append(record)
        ...
        if direction == BACKWARD_WIRE_VALUE:
            if json is None:
        ...
        page = all_recordings[start:end]
        response.json.return_value = {"data": page, "total": total}
        record["returned"] = len(page)
        return response
...
class TestForwardPaginationRegression:
    ...
    def test_102_recordings_four_unique_pages(self, dataset_dict, patched_session):
        ...
        assert [c["returned"] for c in page_calls] == [30, 30, 30, 12]
        ...
        assert set(ids) == {r["id"] for r in recordings}
...
class TestBackwardPagination:
    ...
    def test_initial_backward_request_body_none_with_direction(
        ...
        page_calls = [c for c in calls if c["params"].get("limit") != 1]
        assert page_calls[0]["json"] is None
        assert page_calls[0]["params"]["direction"] == BACKWARD_WIRE_VALUE
    ...
    def test_backward_continuation_body_is_raw_first_recording(
        ...
        for c in page_calls:
            assert c["params"]["direction"] == BACKWARD_WIRE_VALUE
    ...
    def test_direction_wire_value_is_exactly_uppercase_backward(
        self, dataset_dict, patched_session
    ):
        """Every backward request must send the literal string `BACKWARD`.

        The backend validates `direction` against a case-sensitive enum, so
        any other casing turns every backward page into an HTTP 422.
        """
        recordings = _make_recordings(5)
        fake_post, calls = _make_paginated_post(recordings)
        patch_a, patch_b = patched_session(fake_post)
        ...
        sent = [
            c["params"]["direction"] for c in calls if c["params"].get("limit") != 1
        ]
        assert sent
        for value in sent:
            assert isinstance(value, str)
            assert value == "BACKWARD"
    ...
    def test_102_recordings_oldest_to_newest_unique(
        ...
        page_calls = [c for c in calls if c["params"].get("limit") != 1]
        assert len(page_calls) == 4
        assert [c["returned"] for c in page_calls] == [12, 30, 30, 30]

        expected_order = list(reversed([r["id"] for r in recordings]))
        assert reversed_ids == expected_order
        assert len(reversed_ids) == 102
        assert len(set(reversed_ids)) == 102
        assert len(reversed_ids) - len(set(reversed_ids)) == 0
        assert set(reversed_ids) == {r["id"] for r in recordings}
