...
class TestDataset:
    ...
    def test_create_dataset(self, dataset_dict):
        ds = Dataset.create(name=dataset_dict["name"])
        assert isinstance(ds, Dataset)
        assert ds.name == dataset_dict["name"]
        assert ds.num_recordings == 0
    ...
    def test_lazy_loading_of_recordings(self, dataset_dict, recordings_list):
        ds = Dataset(**dataset_dict, recordings=None)
        assert ds.recordings_loaded is False
        assert len(ds.recordings) == 0
