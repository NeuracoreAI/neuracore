...
def test_web_stop_in_a_non_owning_process_flushes_without_publishing_a_stop() -> None:
    ...
    ``StopRecording`` names a source, not a recording, so it closes whatever
    window is open for that source. A process that learned about the recording
    from a notification learns about its stop a whole SSE round trip late, by
    which time a back-to-back recording's window is already open — publishing
    from here killed that next recording. The writer barrier still has to run:
    it is per-process, and this process's tail chunks are sealed nowhere else.
    """
    robot = Robot("robot", instance=0, org_id="org-1")
    robot.id = "robot-id-1"
