...
from neuracore.data_daemon.bridge import LiveRecording
...
def test_an_unanswered_query_falls_back_to_what_this_process_started() -> None:
    """A silent daemon must not read as "not recording".

    `nc.stop_recording` gives up when `is_recording` is False, so a busy or
    absent daemon would otherwise leave a recording this process opened running
    with its stop never published.
    """
    robot = Robot("robot", instance=0, org_id="org-1")
    robot.id = "robot-id-1"
    robot._daemon_recording_context = MagicMock()
