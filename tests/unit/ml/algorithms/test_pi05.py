...
def _resolve_pi05_requirements_file() -> Path:
    repo_root = Path(__file__).resolve().parents[4]

    requirements_file = repo_root / "neuracore/ml/algorithms/pi05/requirements.txt"

    if not requirements_file.exists():
        raise FileNotFoundError(
            f"Could not find pi05 requirements.txt at {requirements_file}"
        )
...
def install_pi05_requirements() -> None:
    """Install requirements.txt from the pi05 folder."""
    requirements_file = _resolve_pi05_requirements_file()
...
def _pi05_env():
    install_pi05_requirements()


@pytest.fixture(scope="module")
def Pi05(_pi05_env):  # noqa: N802
...
def test_run_validation(tmp_path: Path, mock_login, monkeypatch, Pi05):  # noqa: N803
    ...
    monkeypatch.setattr(validate_module.AlgorithmLoader, "load_model", lambda self: Pi05)
