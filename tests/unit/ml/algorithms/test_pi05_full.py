...
PI05_FULL_DEP_PACKAGES = ["transformers", "tokenizers", "huggingface-hub"]


@pytest.fixture(scope="module")
def Pi05Full():  # noqa: N802
    from neuracore.ml.algorithms.pi05_full.pi05 import Pi05Full as Pi05FullModel

    return Pi05FullModel
...
PI05_FULL_TEST_ARGS: dict[str, Any] = {
...
@pytest.fixture
def pytorch_dummy_dataset(Pi05Full) -> PytorchDummyDataset:  # noqa: N803
    input_data_types = Pi05Full.get_supported_input_data_types()
    output_data_types = Pi05Full.get_supported_output_data_types()
...
def test_model_construction(
    model_init_description: ModelInitDescription, Pi05Full
):  # noqa: N803
    model = Pi05Full(model_init_description, **PI05_FULL_TEST_ARGS)
...
def test_run_validation(
    ...
    monkeypatch.setattr(
        validate_module.AlgorithmLoader, "load_model", lambda self: Pi05Full
    )

    # Long timeout due to larger model run on CPU
    os.environ["NEURACORE_ENDPOINT_TIMEOUT"] = "120"
    algorithm_dir = Path(inspect.getfile(Pi05Full)).parent
    _, error_msg = run_validation(
        output_dir=tmp_path,
        algorithm_dir=algorithm_dir,
        port=random.randint(10000, 20000),
        skip_endpoint_check=False,
        algorithm_config=PI05_FULL_TEST_ARGS,
        device=DEVICE,
    )
    if len(error_msg) > 0:
