"""Tests for CQN-AS (Coarse-to-Fine Q-Network with Action Sequence) algorithm."""
...
from neuracore.ml.algorithms.cqn_as.cqn_as import CQN_AS
...
def model_init_description() -> ModelInitDescription:
    """Create model initialization description for CQN-AS."""
    ...
    return BatchedTrainingSamples(
        inputs=BatchedData(
            ...
            rgb_images=MaskableData(
                # CQN-AS expects uint8 images (0-255 range)
...
def test_model_construction(
    ...
    """Test that CQN-AS model can be constructed."""
    model = CQN_AS(model_init_description, **model_config)
...
def test_supported_data_types():
    ...
    input_types = CQN_AS.get_supported_input_data_types()
    output_types = CQN_AS.get_supported_output_data_types()
...
def test_run_validation(tmp_path: Path, mock_login):
    """Test validation run with CQN-AS algorithm."""
    algorithm_dir = Path(inspect.getfile(CQN_AS)).parent
    _, error_msg = run_validation(
        output_dir=tmp_path,
        algorithm_dir=algorithm_dir,
        port=random.randint(10000, 20000),
        algorithm_config=CQN_AS_TEST_ARGS,
        device=DEVICE,
    )
