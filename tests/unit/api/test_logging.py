...
def test_log_target_parallel_gripper_open_amounts(
    temp_config_dir, mock_auth_requests, reset_neuracore, mock_urdf, mocked_org_id
):
    ...
    nc.log_target_parallel_gripper_open_amount(
        name="gripper1",
        value=0.5,
    )
    nc.log_target_parallel_gripper_open_amount(
    ...
    nc.log_target_parallel_gripper_open_amounts({"gripper1": 0.5, "gripper2": 0.7})


def test_log_target_parallel_gripper_open_amount_validation(
    temp_config_dir, mock_auth_requests, reset_neuracore, mock_urdf, mocked_org_id
):
    ...
    with pytest.raises(
        ...
        nc.log_target_parallel_gripper_open_amount(name="gripper1", value="not_a_float")
    ...
    with pytest.raises(
        ...
        nc.log_target_parallel_gripper_open_amount(name="gripper1", value=-0.5)
    ...
    with pytest.raises(
        ...
        nc.log_target_parallel_gripper_open_amount(name="gripper2", value=1.5)

    # Test invalid name type
    with pytest.raises(ValueError, match="Parallel gripper names must be strings"):
        nc.log_target_parallel_gripper_open_amount(name=123, value=0.5)
