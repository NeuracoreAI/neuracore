...
from neuracore.importer.mcap_importer import MCAPDatasetImporter, _TopicImportConfig
...
def test_staged_record_round_trip(tmp_path):
    ...
    expected = [
        ("/topic/a", 1.25, 123, {"value": [1, 2, 3]}),
        ("/topic/b", 2.50, 456, b"\x00\x01"),
    ]
    with staged_path.open("wb") as staged_file:
        for topic, timestamp, log_time_ns, payload in expected:
