...
from neuracore.data_daemon.communications_management.shared_transport.shared_slot_transport import (
    PacketTooLarge,
...
def test_cleanup_trace_written_handles_missing_pending_trace_entry(emitter) -> None:
    bridge = DataBridge(
        comm_manager=DummyComm(),
        recording_disk_manager=DummyRecordingDiskManager(),
        emitter=emitter,
    ...
    bridge._trace_lifecycle.register_trace("rec-1", "trace-1")

    assert bridge.cleanup_channel_on_trace_written("trace-1") is None
    assert bridge._trace_lifecycle.get_trace_metadata("trace-1") == {}


def test_cleanup_expired_channel_drops_registered_trace_without_metadata(
    emitter,
) -> None:
    bridge = DataBridge(
        comm_manager=DummyComm(),
    ...
    bridge._cleanup_expired_channels()

    assert bridge.channels.get("stale-with-trace") is None
    assert bridge._trace_lifecycle._trace_recordings.get_recording_id("trace-1") is None
