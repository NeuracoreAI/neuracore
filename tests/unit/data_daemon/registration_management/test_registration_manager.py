...
class TestGetCloudFileList:
    ...
    def test_rgb_lossy_only_dir_omits_lossless(self, tmp_path) -> None:
        # A trace directory holding only lossy.mp4 (lossy-only / H264_MEDIUM)
        # registers just the lossy video and its sidecar — no lossless.mp4.
...
class TestBatchRegistration:
    ...
    async def test_lossy_only_candidate_payload_omits_lossless(
        ...
        # End-to-end through the candidate's path: a lossy-only trace dir (only
        # lossy.mp4 on disk) must register just the lossy video and its sidecar.
        (tmp_path / "lossy.mp4").write_bytes(b"x")
        session = _mock_http_session({
            "registered_traces": [{"trace_id": "t1", "upload_session_uris": {}}],
            "failed_traces": [],
