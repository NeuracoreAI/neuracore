...
def manager_factory() -> Callable[..., ConnectionManager]:
    ...
    def _make(
        *, offline: bool = False, timeout: float = 2.0, check_interval: float = 1.0
    ):
        return ConnectionManager(
            MockConfigManager(offline=offline),
            timeout=timeout,
...
def test_connection_manager_initializes_offline_mode(
    ...
    manager = manager_factory(offline=True)
    assert manager._running is False
    assert manager._checker_thread is None
...
def test_connection_manager_offline_mode_never_emits_true(
    ...
    manager = manager_factory(offline=True, check_interval=0.1)
