...
def test_non_video_stream_sends_single_data_chunk(monkeypatch) -> None:
    ...
    try:
        ...
        coordinator.enqueue_stream_payload(
            StreamPayload(
                session=session, parts=(memoryview(b"abcd"),), total_bytes=4
            )
        )
        assert _wait_for(
...
def test_batched_joint_enqueues_without_per_stream_channel(monkeypatch) -> None:
    ...
    try:
        ...
        assert _wait_for(
            lambda: any(
                m.command == CommandType.BATCHED_JOINT_DATA for m in messages
            )
        )
    finally:
        coordinator.close()
    ...
    assert (
        batches[0].payload[CommandType.BATCHED_JOINT_DATA.value] == payload.to_dict()
    )
...
def test_stop_recording_freezes_intake_and_sends_trace_end(monkeypatch) -> None:
    ...
    try:
        ...
        coordinator.enqueue_stream_payload(
            StreamPayload(
                session=session, parts=(memoryview(b"efgh"),), total_bytes=4
            )
        )
        time.sleep(0.1)
...
def test_video_stream_routes_over_iox2(monkeypatch, data_type) -> None:
    ...
    try:
        session = _register(
            coordinator, stream_name="camera_0", data_type=data_type
        )
