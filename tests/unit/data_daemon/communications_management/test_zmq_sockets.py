...
def test_deferred_close_honors_zmq_buffer_data(loop_manager: EventLoopManager) -> None:
    ...
    # Create daemon with mocked dependencies (no real sockets needed)
    mock_comm = MagicMock()
    mock_rdm = MagicMock()
    ...
    # Track STOP_RECORDING event emissions
    ...
    try:
        # Create RECORDING_STOPPED message
        msg = MessageEnvelope(
            producer_id=None,
        ...
        # Pre-condition: recording_id not in any set
        assert recording_id not in daemon._pending_close_recordings
        assert recording_id not in daemon._closed_recordings

        # Step 1: Call _handle_recording_stopped
        daemon._handle_recording_stopped(None, msg)

        # Assertion 1: recording_id IN pending, NOT IN closed
        assert recording_id in daemon._pending_close_recordings
        assert recording_id not in daemon._closed_recordings

        # Assertion 2: STOP_RECORDING event emitted exactly once
        assert len(stop_events_received) == 1
        assert stop_events_received[0] == recording_id

        # Step 2: Call _finalize_pending_closes (simulates next loop iteration)
        daemon._finalize_pending_closes()

        # Assertion 3: recording_id IN closed
        assert recording_id in daemon._closed_recordings

        # Assertion 4: _pending_close_recordings is empty after finalization
        assert len(daemon._pending_close_recordings) == 0

        # Assertion 5: State transitions are deterministic - calling again is idempotent
        daemon._finalize_pending_closes()
        assert recording_id in daemon._closed_recordings
        assert len(daemon._pending_close_recordings) == 0
...
def test_data_blocked_after_recording_closed(
    ...
    # Step 1: Directly add recording to closed set (simulates finalized recording)
    daemon._closed_recordings.add(recording_id)

    # Step 2: Create channel with mock ring buffer to track writes
    ...
    # Track events - should NOT receive any
    ...
    try:
        # Step 3: Create DATA_CHUNK message for closed recording
        ...
        # Step 4: Call _handle_write_data_chunk with logging capture
        with caplog.at_level(logging.WARNING):
            daemon._handle_write_data_chunk(channel, msg)

        # Assertion 1: Method returns early - ring buffer NOT written to
        mock_ring_buffer.write.assert_not_called()

        # Assertion 2: Warning logged with correct recording_id and trace_id
        warning_found = any(
        ...
        # Assertion 3: No events emitted
        assert len(events_received) == 0

        # Assertion 4: Channel state unchanged (trace_id set before guard check)
        # Note: trace_id IS set at line 434 before the closed check at line 436
        # This is the current behavior - channel.trace_id gets updated
...
def test_channel_expires_after_timeout_without_recording_stopped(
    ...
    # Step 1: Create channel with mock ring buffer
    mock_ring_buffer = MagicMock()
    ...
    # Step 2: Set last_heartbeat to expired (15 seconds ago, timeout is 0.3s in tests)
    channel.last_heartbeat = datetime.now(timezone.utc) - timedelta(seconds=15)

    # Register channel and trace mapping
    daemon.channels[producer_id] = channel
    ...
    # Pre-condition: channel exists
    assert producer_id in daemon.channels

    # Step 3: Call _cleanup_expired_channels
    daemon._cleanup_expired_channels()

    # Assertion 1: Channel with expired last_heartbeat is removed from channels dict
    assert producer_id not in daemon.channels

    # Assertion 2: _handle_end_trace was triggered (via _on_complete_message to RDM)
    # The RDM's enqueue method should have been called with final_chunk=True
    mock_rdm.enqueue.assert_called()
    call_args = mock_rdm.enqueue.call_args
    # CompleteMessage is passed as first positional arg
    ...
    # Assertion 3: Ring buffer deleted (channel removed = buffer dereferenced)
    # The channel object is deleted from daemon.channels, so ring buffer is dereferenced

    # Assertion 4: Channel not in daemon.channels after cleanup (already asserted above)

    # Assertion 5: Cleanup is idempotent - calling again doesn't error
    daemon._cleanup_expired_channels()  # Should not raise
    assert producer_id not in daemon.channels  # Still not present
...
def test_channel_stays_alive_within_timeout(loop_manager: EventLoopManager) -> None:
    ...
    # Step 2: Set last_heartbeat to recent (0.1s ago, within 0.3s timeout)
    ...
    # Assertion 1: Channel with recent last_heartbeat remains in channels dict
    assert producer_id in daemon.channels

    # Assertion 2: NO TRACE_END flow triggered (RDM.enqueue not called)
    mock_rdm.enqueue.assert_not_called()

    # Assertion 3: Ring buffer unchanged and functional
    assert channel.ring_buffer is initial_ring_buffer
    assert channel.ring_buffer is mock_ring_buffer

    # Assertion 4: Channel state (trace_id) preserved
    assert channel.trace_id == initial_trace_id
    assert channel.trace_id == trace_id
    # recording_id is in _trace_recordings, not on channel directly
    assert daemon._trace_recordings[trace_id] == recording_id

    # Assertion 5: Multiple calls to cleanup don't affect active channels
    daemon._cleanup_expired_channels()
    daemon._cleanup_expired_channels()
    assert producer_id in daemon.channels
    assert channel.trace_id == trace_id
    assert channel.ring_buffer is mock_ring_buffer
    mock_rdm.enqueue.assert_not_called()  # Still never called
