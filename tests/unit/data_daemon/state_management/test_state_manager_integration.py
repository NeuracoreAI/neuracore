...
@pytest.fixture
def manager_store(tmp_path) -> tuple[StateManager, SqliteStateStore]:
...
def _get_trace_status(store: SqliteStateStore, trace_id: str) -> TraceStatus:
    with store._engine.begin() as conn:
        return conn.execute(
...
def test_trace_written_emits_ready_and_progress_report(manager_store) -> None:
    ...
    _set_created_at(store, "trace-1", created_early)
    _set_created_at(store, "trace-2", created_late)
...
def test_offline_then_connected_emits_ready(manager_store) -> None:
    manager, store = manager_store
    manager.create_trace(
        "trace-offline",
        "rec-offline",
...
def test_uploaded_bytes_updates_store(manager_store) -> None:
    ...
    trace = store.get_trace("trace-uploaded")
    assert trace is not None
    assert trace.bytes_uploaded == 5


def test_invalid_transition_raises_via_manager(manager_store) -> None:
    ...
    with pytest.raises(ValueError, match="Invalid status transition"):
        manager.update_status("trace-invalid", TraceStatus.UPLOADED)


def test_multiple_state_managers_share_sqlite_db(tmp_path) -> None:
...
def test_concurrent_writes_with_wal(tmp_path) -> None:
    db_path = tmp_path / "state.db"
    store_one = SqliteStateStore(db_path)
    store_two = SqliteStateStore(db_path)
    ...
    with store_one._engine.begin() as conn:
    ...
    assert errors == []
    trace = store_one.get_trace("trace-concurrent")
    assert trace is not None
    assert trace.bytes_uploaded in {10, 20}


def test_full_state_transition_chain(manager_store) -> None:
    manager, store = manager_store
    manager.create_trace(
        "trace-chain",
        "rec-chain",
    ...
    manager.update_status("trace-chain", TraceStatus.WRITING)
    emitter.emit(Emitter.TRACE_WRITTEN, "trace-chain", "rec-chain", 12)

    claimed = manager.claim_ready_traces(limit=1)
    assert [row["trace_id"] for row in claimed] == ["trace-chain"]
    assert claimed[0]["status"] == TraceStatus.UPLOADING

    manager.update_status("trace-chain", TraceStatus.UPLOADED)
    assert _get_trace_status(store, "trace-chain") == TraceStatus.UPLOADED


def test_race_conditions_on_rapid_state_changes(tmp_path, caplog) -> None:
    """Test race conditions on rapid state changes.

    Two threads concurrently update the state of the same trace.
    The first thread will update the state to WRITTEN, then UPLOADING, then UPLOADED.
    The second thread will update the state to WRITTEN, then UPLOADING, then UPLOADED.
    ...
    manager_one.update_status("trace-race", TraceStatus.WRITING)

    barrier = threading.Barrier(2)
    errors: list[str] = []

    def worker(manager: StateManager) -> None:
        try:
            barrier.wait()
            manager.update_status("trace-race", TraceStatus.WRITTEN)
            assert _get_trace_status(store_one, "trace-race") == TraceStatus.WRITTEN
            manager.update_status("trace-race", TraceStatus.UPLOADING)
            assert _get_trace_status(store_one, "trace-race") == TraceStatus.UPLOADING
            manager.update_status("trace-race", TraceStatus.UPLOADED)
            assert _get_trace_status(store_one, "trace-race") == TraceStatus.UPLOADED
        except ValueError as exc:
            errors.append(str(exc))

    try:
        with caplog.at_level(logging.INFO):
            thread_one = threading.Thread(target=worker, args=(manager_one,))
            thread_two = threading.Thread(target=worker, args=(manager_two,))
            thread_one.start()
            thread_two.start()
            thread_one.join()
            thread_two.join()
...
def test_state_recovery_after_restart(tmp_path) -> None:
    ...
    try:
        ...
        manager.update_status("trace-recover", TraceStatus.WRITING)
...
def test_ready_for_upload_includes_bytes_after_reconnect(manager_store) -> None:
    manager, store = manager_store
    manager.create_trace(
        "trace-resume",
        "rec-resume",
...
def test_ready_for_upload_includes_bytes_after_restart(tmp_path) -> None:
    db_path = tmp_path / "state.db"
    store = SqliteStateStore(db_path)
    manager = StateManager(store)
    try:
...
def test_failed_trace_not_claimed_for_retry(manager_store) -> None:
    manager, _ = manager_store
    manager.create_trace(
        "trace-failed",
        "rec-failed",
    ...
    manager.update_status("trace-failed", TraceStatus.FAILED)

    claimed = manager.claim_ready_traces(limit=1)
    assert claimed == []


def test_simultaneous_recordings_emit_progress_reports(manager_store) -> None:
    manager, _ = manager_store
    ...
    try:
        ...
        assert progress_event.wait(timeout=1.0)
        recording_sets = [
            {trace.recording_id for trace in traces} for _, _, traces in progress_events
        ]
        assert {frozenset(ids) for ids in recording_sets} == {
            frozenset({"rec-a"}),
            frozenset({"rec-b"}),
        }
    finally:
...
def test_resume_after_failure_and_reconnect(manager_store) -> None:
    manager, store = manager_store
    manager.create_trace(
        "trace-e2e",
        "rec-e2e",
    ...
    trace = store.get_trace("trace-e2e")
    assert trace is not None
    ...
    try:
        ...
        assert store.get_trace("trace-e2e") is None
    finally:
        emitter.remove_listener(Emitter.READY_FOR_UPLOAD, ready_handler)
        emitter.remove_listener(Emitter.DELETE_TRACE, delete_handler)


def test_encoder_crash_does_not_block_other_recordings(manager_store) -> None:
    manager, _ = manager_store
    manager.create_trace(
        "trace-a",
        "rec-a",
    ...
    try:
        ...
        assert ready_events
        assert ready_events[-1][:2] == ("trace-b", "rec-b")
    finally:
        emitter.remove_listener(Emitter.READY_FOR_UPLOAD, ready_handler)
