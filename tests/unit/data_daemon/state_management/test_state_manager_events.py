...
import neuracore.data_daemon.state_management.state_manager as state_manager_module
...
class FakeStateStore:
    def __init__(self) -> None:
        ...
        self._recording_stopped_at: dict[str, datetime] = {}
    ...
    async def set_stopped_at(self, recording_id: str) -> None:
        ...
        self._recording_stopped_at[recording_id] = now
    ...
    async def get_recording_stopped_at(self, recording_id: str) -> datetime | None:
        return self._recording_stopped_at.get(recording_id)
...
@pytest.mark.asyncio
async def test_stop_recording_requested_sends_ack_with_persisted_stop_time(
    emitter: Emitter,
    tmp_path,
) -> None:
    store = FakeStateStore()
    ...
    socket_path = tmp_path / "recording_stop_ack.ipc"
    endpoint = f"ipc://{socket_path}"
    pull = zmq_context.socket(zmq.PULL)
    pull.bind(endpoint)

    try:
        emitter.emit(Emitter.STOP_RECORDING_REQUESTED, "rec-ack", endpoint)
        await asyncio.sleep(0.2)
        ...
        payload = json.loads(pull.recv().decode("utf-8"))
        assert payload["recording_id"] == "rec-ack"
        assert payload["stopped_at"] > 0
        assert (
            store._recording_stop_report_status["rec-ack"]
            == RecordingStopReportStatus.PENDING
        )
    finally:
        pull.close(0)
...
@pytest.mark.asyncio
async def test_report_recording_stop_posts_json_body_with_persisted_end_time(
    state_manager, monkeypatch
) -> None:
    manager, store = state_manager
    stopped_at = datetime(2024, 1, 2, 3, 4, 5, tzinfo=timezone.utc)
    store._recording_stopped_at["rec-stop-body"] = stopped_at
    store._recording_stop_report_status["rec-stop-body"] = (
        RecordingStopReportStatus.PENDING
    )
    ...
    with (
        patch(
            "neuracore.data_daemon.state_management.state_manager.get_auth",
            return_value=mock_auth,
        ),
        patch(
            "neuracore.data_daemon.state_management.state_manager.get_current_org",
            return_value="org-123",
        ),
        patch(
            "neuracore.data_daemon.state_management.state_manager.aiohttp.ClientSession",
            return_value=mock_session,
        ),
    ):
        reported = await manager._report_recording_stop("rec-stop-body")

    assert reported is True
    mock_session.post.assert_called_once_with(
        f"{state_manager_module.API_URL}/org/org-123/recording/stop",
        json={
            "recording_id": "rec-stop-body",
            "end_time": float(stopped_at.timestamp()),
        },
