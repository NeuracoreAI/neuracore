...
class Timer:
    ...
    def __exit__(self, *args):
        ...
        if self.label:
            ...
            if should_log:
                level = logging.WARNING if self.interval >= self.max_time else logging.INFO
                logger.log(
                    level,
...
def verify_dataset(config, expected_frame_count):
    ...
    with Timer(max_time=config.stop_wait_timeout_s, label="nc.get_dataset", always_log=True):
        dataset = nc.get_dataset(config.dataset_name)
    with Timer(max_time=MAX_TIME_TO_SAVE_S, label="dataset.synchronize", always_log=True):
...
def run_streaming_test(config:TestConfig):
    ...
    with Timer(max_time=config.stop_wait_timeout_s, label="run_streaming_test.total", always_log=True):
        with Timer(max_time=MAX_TIME_TO_START, label="nc.login", always_log=True):
            nc.login()
        with Timer(max_time=MAX_TIME_TO_START, label="nc.connect_robot", always_log=True):
            nc.connect_robot(
                config.robot_name,
                urdf_path=str(BIMANUAL_VIPERX_URDF_PATH),
                overwrite=False,
            )
        with Timer(max_time=MAX_TIME_TO_START, label="nc.create_dataset", always_log=True):
            nc.create_dataset(
                config.dataset_name,
                description=(
        ...
        with Timer(max_time=config.stop_wait_timeout_s, label="pre-verify sleep", always_log=True):
...
def test_high_bandwidth():
    ...
    # Set daemon bandwidth limit above generated throughput for this stress test.
    # This test is meant to validate recording behavior at high ingest rates,
    # not upload throttling behavior.
