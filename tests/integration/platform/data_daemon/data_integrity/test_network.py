...
def test_split_process_video_survives_recording_boundary() -> None:
    ...
    with (
        ...
        try:
            with (
                scoped_holdback_ms(_BOUNDARY_RACE_HOLDBACK_MS),
                online_daemon_running(),
            ):
                assert_exactly_one_daemon_pid()
...
def test_split_process_video_survives_back_to_back_recording_boundaries() -> None:
    ...
    Runs one :func:`~runners.split_video_process_running` child across
    ``case.recording_count`` recordings started back-to-back, and classifies
    each recording's frames separately from the wall-clock brackets around its
    own control calls — mirroring how ``context_worker`` builds
    ``bounds_by_disk_key`` per recording.
