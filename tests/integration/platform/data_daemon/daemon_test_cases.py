...
NETWORK_ONLY_INTEGRITY_CASES = (
    ...
    # Remote control at the heaviest cross-process shape, eight windows deep.
...
SYNC_MEMORY_JOINTS_PER_FRAME_CASES = tuple(
    replace(_SYNC_MEMORY_CONTROL, joint_count=joints) for joints in (28, 56, 112, 224)
)
...
SYNC_MEMORY_RECORDINGS_PER_DATASET_CASES = tuple(
    replace(_SYNC_MEMORY_CONTROL, recording_count=count) for count in (4, 8, 24)
)

# A production-shaped combination of a longer episode, a wider joint payload,
# and several camera traces. Run it three times to expose run-to-run variance
# without changing the requested 18-case / 51-recording experiment budget.
SYNC_MEMORY_COMBINED_STRESS_CASES = tuple(
    replace(
        _SYNC_MEMORY_CONTROL,
        duration_sec=160,
        joint_count=28,
...
SYNC_MEMORY_CASES = (
    ...
    *SYNC_MEMORY_COMBINED_STRESS_CASES,
)

# Keep the workload and backend analyzer defaults in lockstep. These assertions
# fail at collection time if a future case edit silently changes the experiment.
assert len(SYNC_MEMORY_CASES) == 18
assert sum(case.recording_count for case in SYNC_MEMORY_CASES) == 51
