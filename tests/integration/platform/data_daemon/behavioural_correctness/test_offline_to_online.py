    ...
    try:
        with scoped_storage_state(case, dataset_name=dataset_name):
            with offline_daemon_running():
                ...
                # Offline contract: rows exist by recording_index with the cloud
                # recording_id still NULL (minted on demand at upload time).
            ...
            with online_daemon_running():
                # Daemon-DB upload completion is keyed by the active correlation
                # key: the local recording_index under the Rust daemon, the cloud
                ...
                # Under the Rust daemon the start-notifier / mint-on-demand fills
                # the cloud recording_id once online; resolve it so cloud
                ...
                # The daemon now POSTs the producer's real captured start/stop
                # timestamps, so an offline recording recovered after going
                # online reports its true duration (no longer collapsed to ~0s
                # by the back-to-back /recording/start and /recording/stop POSTs).
