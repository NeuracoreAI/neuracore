...
Far enough that a window pinned to it could hold nothing published today.
"""
...
def _record_one(
    ...
    *,
    ...
    Frames are logged synchronously between the control calls, so every one is
...
def test_explicit_capture_timestamps_are_stored_and_leave_the_window_alone() -> None:
    ...
    The row must carry the far-off capture clock exactly while every frame still
    lands: a window pinned to it would route every sample out of window.
    """
    ...
    with scoped_test_dir_state(_CASE):
