...
from tests.integration.platform.data_daemon.shared.reporting import PerformanceReport
...
def test_cloud_upload_and_readiness_performance(
    ...
    performance_report.parameter("dataset", dataset_name)
    performance_report.parameter("recordings", case.recording_count)
    performance_report.parameter("parallel contexts", case.parallel_contexts)
    performance_report.parameter("recording duration (s)", case.duration_sec)
    performance_report.parameter("joint FPS", case.joint_fps)
    performance_report.parameter("video FPS", case.video_fps)
    performance_report.parameter("video streams", case.video_count)
    performance_report.parameter("stop waits for readiness", case.wait)
    ...
    try:
        with scoped_storage_state(case, dataset_name=dataset_name):
            with online_daemon_running():
                with performance_report.step("Record workload and stop recordings"):
                    with Timer(
                ...
                with performance_report.step("Wait for cloud dataset readiness"):
                    with Timer(
                        case_timeout_seconds(case),
        ...
        # Register after scoped cleanup so end-to-end wall time includes daemon
        # shutdown, local cleanup, and cloud dataset deletion.
