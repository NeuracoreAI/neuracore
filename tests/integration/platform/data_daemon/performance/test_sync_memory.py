...
Records a workload and emits one ``SYNC_BENCH_CASE`` line carrying the source
dataset ID before explicitly synchronising it. The memory itself is measured
server-side by ``neuracore_shared.synchronization._memory_diag`` and read back
out of Cloud Logging afterwards::
...
The shared source dataset ID in ``SYNC_BENCH_CASE`` and ``SYNC_MEM_DIAG`` lets
the analyser attribute every server-side sample exactly; without it the samples
are just an undifferentiated cloud of points.
...
def _independent_variable(case: DataDaemonTestCase) -> str:
    ...
    Returns:
        ...
        ``combined_stress`` when the deliberately combined case moves several.
    """
    moved = []
    ...
    if not moved:
        return "control"
    return moved[0] if len(moved) == 1 else "combined_stress"
...
def _emit_case_record(
    ...
    """Log case parameters before its synchronization tasks are launched."""
    record = {
        "marker": BENCH_MARKER,
    ...
    with performance_report(case, dataset_name=dataset_name) as report:
        ...
        with scoped_storage_state(case, specs):
            with online_daemon_running():
                ...
                with report.step("Synchronize cloud dataset"):
                    with Timer(
                        case_timeout_seconds(case),
                        label="sync_memory.dataset_synchronize",
                        always_log=True,
                    ):
                        dataset = nc.get_dataset(results[0].dataset_name)
                        ...
                        assert len(synchronized_dataset) == case.recording_count
