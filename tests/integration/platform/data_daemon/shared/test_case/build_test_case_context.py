...
class FrameReport:
    ...
    Filled from one thread per stream, so appends are serialised. Read only
    once those threads have joined.
    """
...
def recording_timestamps(
    ...
    A pure function of ``(spec, recording_index)``, sharing nothing with the
    schedule the producers emit from (:func:`stream_frame_schedule`), so a
    producer scoped to one recording can be checked against the sequence it was
    supposed to log rather than against itself.
    """
    case = spec.case
