...
def preallocate_depth_buffer(
    ...
    Callers that log depth frames reuse a single buffer across iterations
    (via :func:`encode_depth_frame`'s ``out`` param) instead of allocating a
    new array per frame.
...
def context_worker(spec: ContextSpec) -> ContextResult:
    ...
    _depth_camera_name_list = depth_camera_names(
        case.depth_count
    )  # preserved for depth logging port
