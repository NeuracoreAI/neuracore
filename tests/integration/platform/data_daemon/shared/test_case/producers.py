...
def _producer_process(
    ...
    continuously and the daemon's window announcement decides what is kept.
...
class MultiProcessProducerSession(ProducerSession):
    ...
    children never call ``start_recording``, learning that a window exists from
    the daemon's own announcement — the topology a real deployment has when
    camera and recording owner are separate, and it needs no cloud."""
