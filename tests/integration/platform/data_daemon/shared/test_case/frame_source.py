...
Every frame the test producers log — RGB or depth — is built here, so the
producer modules stay orchestration and the pixel content has one home.

RGB frames must cost the daemon's video pipeline what a real camera costs it. A
solid-colour frame does not: it compresses ~620:1 losslessly, leaving the SDK's
PNG pool, the video spool and the ``libx264rgb`` encoder idle, so the suite can
say nothing about real-time viability.
...
Depth frames (:func:`encode_depth_frame`) answer a different question — the
round trip through the 24-bit depth-to-RGB encoding, not encoder throughput — so
they are a cheap analytic pattern rather than a bank, and carry their identity in
their values rather than in a painted grid. Both kinds draw their per-frame
identity from the same :func:`frame_code_base`.
"""
...
def encode_depth_frame(
    ...
    - ``base(frame_num)`` — a pseudo-random-looking but deterministic
      per-frame offset, keyed off the same composite frame code the RGB
      producer paints into its frames (:func:`frame_code_base` plus the frame
      index) — so distinct frames/cameras/recordings never collide on the same
      pattern.
    ...
    Unlike the RGB path there is no frame bank: the pattern is a couple of
    broadcast adds over preallocated ramps, so building it per frame costs less
    than copying one out of a bank would.
...
def preallocate_depth_buffer(
    ...
    The depth counterpart to :func:`make_camera_feed`: callers reuse a single
    buffer across iterations (via :func:`encode_depth_frame`'s ``out`` param)
    instead of allocating a new array per frame. A plain buffer rather than a
    feed object, because depth content is generated per frame rather than
    copied out of a bank.
