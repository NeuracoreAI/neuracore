...
per-test artifact directory setup, and the :func:`scoped_test_dir_state` /
...
@contextmanager
def scoped_test_dir_state(
    case: DataDaemonTestCase,
    *,
    dataset_name: str | None = None,
) -> Generator[None]:
    """Apply case-scoped storage and daemon-log cleanup before and after the block.
    ...
    Wraps the block in :func:`scoped_daemon_log_action` so the shared
    ``daemon.log`` is cleaned up on the same schedule.

    Does **not** start, stop, or signal any daemon process.

    Args:
        case: Test case whose ``storage_state_action`` controls DB/recordings
            cleanup and whose ``daemon_log_action`` controls the daemon log.
...
def scoped_daemon_log_action(case: DataDaemonTestCase) -> Generator[None]:
    ...
    Args:
        case: Test case whose ``daemon_log_action`` controls cleanup.
