...
def assert_context_mode(case: DataDaemonTestCase, results: list[ContextResult]) -> None:
    ...
    # A result is "active" if it produced at least one recording. Under the Rust
    # daemon liveness is keyed off recording_indexes (always populated by the
    # daemon; the cloud recording_ids may be empty strings until minted offline).
    # Under the legacy daemon the cloud recording_ids are authoritative.
...
def _verify_recording_structure(
    *,
    ...
    # The daemon now POSTs the producer's real captured window boundaries
    # (start_timestamp_ns / stop_timestamp_ns) as start_time / end_time, so even
    # a recording recovered after going online reports its true span rather than
    # collapsing to ~0s from the back-to-back /recording/start and
    # /recording/stop receipt times.
