...
def _explain_surplus(values: list[float], condemned: dict[float, str]) -> str:
    ...
    The reasons are categories, so traces that disagree the same way share one
    string and still collapse into a single failure line.
    """
...
def _locate_count_mismatch(
    ...
    Where the difference falls names the defect: before or after the
    expected span, a duplicate, or a genuine shortfall.
    """
...
def assert_disk_recording_properties(
    ...
    for result in results:
        for recording_key, fetch_key in _result_recording_keys(result):
            ...
            # A trace owed frames but never written escapes the loop above;
            # skipped when empty, since a late starter can own none of a recording.
