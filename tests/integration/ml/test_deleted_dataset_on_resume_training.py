...
def _assert_recordings_deleted(recordings: list) -> None:
    """Verify recordings owned by the deleted dataset are no longer accessible."""
    # Keep the Recording objects captured before deletion. Refreshing each
    # object's metadata makes a direct API request and therefore proves the
    # records are gone rather than hidden only from dataset listings.
    pending_recordings = list(recordings)
    deadline = time.time() + DELETION_TIMEOUT_SECONDS
    while pending_recordings and time.time() < deadline:
        still_accessible = []
        for recording in pending_recordings:
            try:
                recording._refresh_recording_metadata()
            except RuntimeError:
                # Recording._refresh_recording_metadata wraps the backend 404
                # in RuntimeError, which is the expected post-deletion result.
                continue
            still_accessible.append(recording)
        pending_recordings = still_accessible
        if pending_recordings:
            time.sleep(DELETION_POLL_SECONDS)

    assert not pending_recordings, (
        "Recordings remained accessible after dataset deletion: "
        f"{[recording.id for recording in pending_recordings]}"
    )
...
def test_resume_fails_when_training_dataset_has_been_deleted() -> None:
    ...
    try:
        ...
        # The merge_datasets call returns as soon as the new dataset record is created,
        dataset = nc.merge_datasets(
            name=dataset_name,
            dataset_names=[SOURCE_DATASET_NAME],
        )
        ...
        _assert_recordings_deleted(recordings)
        dataset = None
