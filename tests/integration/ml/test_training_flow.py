...
def _run_policy_inference(policy: Policy) -> None:
    ...
    try:
        ...
        _assert_prediction_contract(predictions, inference_path="explicit sync point")
        ...
        _assert_prediction_contract(predictions, inference_path="logged sensor data")
...
def _assert_prediction_contract(
    ...
    *,
    ...
    """Validate endpoint output names, tensor shape, and numeric contents."""
    for data_type, indexed_names in OUTPUT_EMBODIMENT_DESCRIPTION.items():
        assert data_type in predictions, (
            f"{inference_path}: expected {data_type.value}, "
            f"got: {list(predictions.keys())}"
        )
        expected_names = set(indexed_names.values())
        actual_outputs = predictions[data_type]
        assert set(actual_outputs) == expected_names, (
            f"{inference_path}: unexpected {data_type.value} output names; "
            f"expected={sorted(expected_names)}, got={sorted(actual_outputs)}"
        )

        value_field = (
            "value" if data_type == DataType.JOINT_TARGET_POSITIONS else "open_amount"
        )
        for name, output in actual_outputs.items():
            tensor = getattr(output, value_field)
            assert isinstance(tensor, torch.Tensor), (
                f"{inference_path}: {data_type.value}/{name} returned "
                f"{type(tensor).__name__}, expected torch.Tensor"
            )
            assert tuple(tensor.shape) == (
                1,
                CNNMLP_CONFIG["output_prediction_horizon"],
                1,
            ), (
                f"{inference_path}: {data_type.value}/{name} returned shape "
                f"{tuple(tensor.shape)}"
            )
            assert torch.isfinite(
                tensor
            ).all(), f"{inference_path}: {data_type.value}/{name} contains NaN or Inf"
...
class TestTrainingFlow:
    ...
    def teardown_class(cls) -> None:
        if not cls.all_steps_passed:
            logger.warning(
                "One or more TestTrainingFlow steps failed; cleaning up any "
    ...
    def test_step9_deploy_remote_endpoint(self) -> None:
        ...
        assert (
            str(final_status).strip().lower() == ENDPOINT_ACTIVE_STATE
        ), f"Endpoint did not become active, status: {final_status!r}"
        nc.connect_robot(robot_name=MUJOCO_ROBOT_NAME)
        policy = nc.policy_remote_server(endpoint_name)
        _run_policy_inference(policy=policy)
        logger.info(
            f"[STEP 9] [PASSED] Endpoint {self.endpoint_id} Is Active And "
            "Remote Inference Succeeded"
        )
