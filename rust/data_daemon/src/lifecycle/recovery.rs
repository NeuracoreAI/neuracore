...
pub async fn run_startup_sweeps(store: &SqliteStateStore, recordings_root: &Path) {
    // The claim/drain queries that drive the coordinators only scan
    // terminal-or-pending rows, so re-arming the transient `registering` /
    // `uploading` rows is what stops a SIGKILL mid-upload from leaking traces.
...
async fn sweep_partial_recordings(
    ...
    for recording in recordings {
        ...
        // Only a recording whose every trace finished writing is preserved. A
        // recording with no trace rows yet (created by `/recording/start` but
        // killed before the producer wrote anything) fails the `!is_empty()`
        // guard and is treated as partial — purged below.
