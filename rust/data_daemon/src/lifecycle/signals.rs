...
/// A handle to the shutdown channel: clone [`subscribe`](Self::subscribe) for
/// each task that needs to wait for shutdown.
#[derive(Clone)]
...
/// [`ShutdownHandle`] and the *primary* shutdown receiver that the caller
/// must await.
///
...
pub fn install_shutdown_handler(
) -> std::io::Result<(ShutdownHandle, broadcast::Receiver<ShutdownSignal>)> {
    ...
    Ok((ShutdownHandle { sender }, primary_receiver))
}
...
mod tests {
    ...
    async fn explicit_signal_reaches_subscriber() {
        ...
        let (sender, primary_receiver) = broadcast::channel(8);
        let handle = ShutdownHandle { sender };
