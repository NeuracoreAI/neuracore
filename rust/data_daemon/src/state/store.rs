...
pub trait StateStore: Send + Sync {
    ...
    /// terminal stamp — `stopped_at` and `cancelled_at` both NULL — however the
    /// recording began, so an SDK process gets the same answer for one it
    /// started itself and one started from the web.
    ///
    /// The newest row wins rather than erroring, so one left unstamped by an
    /// earlier crash cannot mask the live recording behind it.
