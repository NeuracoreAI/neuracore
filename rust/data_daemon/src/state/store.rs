...
///
/// With the read/write pool split only one connection ever writes, but a
/// background WAL checkpoint or an external reader can still briefly hold the
/// database lock; 5 s comfortably absorbs those transient holds without masking
/// a genuine deadlock.
...
pub trait StateStore: Send + Sync {
    ...
    /// Returns `Ok(())` whether or not a row matched (a missing `trace_id` is a
    /// no-op `UPDATE`).
    ...
    /// `(robot_id, robot_instance, start_timestamp)`.
    ///
    /// Backs the `queries` IPC service: the SDK asks the daemon for the id
    /// instead of reading this DB directly. Matches `start_timestamp`
    /// exactly (the caller's start tick, stored verbatim), and also
    /// `start_publish_timestamp_ns` when given, and excludes
    ...
    async fn resolve_recording_id_for_marker(
        ...
        robot_instance: i64,
        start_timestamp: i64,
        start_publish_timestamp_ns: Option<i64>,
    ...
    /// update and the number of write-phase trace rows transitioned to `failed`
    /// (the later upload/registration burns can touch further rows that this
...
/// unit the [`crate::state::trace_event_database_writer::TraceEventDatabaseWriter`] batcher flushes.
///
...
impl SqliteStateStore {
    ...
    ///
    /// The [`crate::state::trace_event_database_writer::TraceEventDatabaseWriter`] batcher coalesces many
    /// actors' per-frame progress / status / finalise updates (last-writer-wins
    /// per trace) and flushes them together here, so the per-transaction
    /// (begin/commit) cost is amortised across the whole batch instead of paid
    /// per row. Each SET clause is a fixed `COALESCE(?, col)`
...
/// `StateStore` for [`SqliteStateStore`] — see the struct for the read/write
/// connection-pool split. The per-frame hot path never calls these methods
/// directly: actor updates funnel through the coalescing write-behind batcher
/// ([`crate::state::trace_event_database_writer::TraceEventDatabaseWriter`]) and
/// are applied here as one batched transaction.
#[async_trait]
    ...
    async fn resolve_recording_id_for_marker(
        ...
        let recording_id = sqlx::query_scalar::<_, Option<String>>(
            "SELECT recording_id FROM recordings \
              WHERE robot_id = ?1 AND robot_instance = ?2 AND start_timestamp = ?3 \
                AND (?4 IS NULL OR start_publish_timestamp_ns = ?4) \
                AND cancelled_at IS NULL \
        ...
        .bind(start_publish_timestamp_ns)
        .fetch_optional(&self.read_pool)
        .await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn the_start_publish_time_tells_recordings_with_one_marker_apart() {
        // An importer starts every episode at tick 0. The previous episode's
        // row must not answer for the next one before its row exists.
        let (store, _tempdir) = open_store().await;
        let start = |publish_timestamp_ns| NewRecording {
        ...
        let first = store.create_recording(start(100)).await.unwrap();
        store
            .mark_recording_start_notified(first.recording_index, "cloud-first")
            .await
            .unwrap();

        assert_eq!(
            store
                .resolve_recording_id_for_marker("robot-1", 0, 0, Some(200))
                .await
                .unwrap(),
            None,
            "the second episode's row does not exist yet"
        );

        let second = store.create_recording(start(200)).await.unwrap();
        store
            .mark_recording_start_notified(second.recording_index, "cloud-second")
            .await
            .unwrap();
        for (publish_timestamp_ns, expected) in [
            (Some(100), "cloud-first"),
            (Some(200), "cloud-second"),
            (None, "cloud-second"),
        ] {
            assert_eq!(
                store
                    .resolve_recording_id_for_marker("robot-1", 0, 0, publish_timestamp_ns)
                    .await
                    .unwrap()
                    .as_deref(),
                Some(expected)
    ...
    async fn claim_for_registration_respects_size_trigger() {
        ...
        // Size trigger: 5 ready ≥ limit of 3, so the oldest 3 are claimed even
        // though last_updated is fresh.
        let claimed = store
