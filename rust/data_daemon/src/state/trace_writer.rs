...
//!
//! Per-trace actors emit a high-frequency stream of small column updates: a
//! `writing` status bump on the first frame, a debounced `bytes_written` every
//! N frames, the finalise `written` + `total_bytes`, or a `failed`. Writing
//! each as its own SQLite transaction under the store's single `write_guard`
//! made ~96% of the daemon's DB-layer time pure mutex contention (the
//! log-latency profile): the actual per-row work was ~430 µs but writers spent
//! ~10 ms each *waiting* for the guard, and that wait balloons under CPU
//! starvation because the guard is held across the transaction's awaits.
//!
//! This task removes the actor↔DB coupling entirely:
//!
//! - actors `send` a partial update and never await a transaction (lever: the
//!   actor's hot path no longer touches SQLite at all);
//! - consecutive ops for the same trace are **coalesced** last-writer-wins per
//!   column, so a burst of `bytes_written` updates collapses to one row write
//!   (lever 1 + 5: progress is write-behind and deduplicated);
//! - pending writes flush in a single **batched** transaction
//!   ([`SqliteStateStore::apply_trace_writes`]) on a short timer or once the
//!   pending set grows past a cap (lever 2 + 3: one `write_guard` acquisition,
//!   fixed statements, no read-back `SELECT`).
//!
...
/// How often pending writes are flushed. Small enough that a finalised trace
/// becomes visible to the registration coordinator promptly (its claim policy
/// debounces in seconds, so this is negligible), large enough that a burst of
/// progress updates coalesces into one row write per flush.
...
/// [`TraceWriter::shutdown`] to drain, flush, and join.
...
/// actors plus the [`TraceWriter`] owner for shutdown.
