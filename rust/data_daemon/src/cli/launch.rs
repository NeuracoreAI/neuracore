...
/// requests are rare (one per `RefreshConfig` command) and each is drained
/// promptly, so a small buffer is ample; it only needs slack so a burst of
/// back-to-back commands never blocks the dispatcher on send.
