...
/// the uploader's burst of concurrent per-file PUTs:
/// - `MAX_IDLE_PER_HOST` matches the uploader's `MAX_CONCURRENT_UPLOADS` so a
///   full burst can each keep its connection warm for the next file instead of
///   being evicted between files.
/// - `IDLE_TIMEOUT` keeps an idle connection warm across the gap between
///   consecutive recordings, so the next burst reuses the pool rather than
///   re-handshaking TLS.
/// - `TCP_KEEPALIVE` holds a warm-but-idle path open (and detects a dead one)
///   rather than letting it silently go half-open.
...
impl ApiClient {
    ...
    /// Build the `Authorization` header map applied to every outgoing request.
    ///
    /// Fetches a fresh bearer token from the auth provider and formats it as
    /// `Authorization: Bearer <token>`. The token comes from user-controlled
    /// config, so a value that cannot be encoded into a header byte string is
    /// surfaced as [`ApiClientError::Decode`]. This is the only header set at the
    /// client level — bodied `POST`/`PUT` calls add their own
    /// `Content-Type: application/json` via the request builder.
