...
//!
//! The output file is created (and truncated) on [`JsonTraceWriter::open`]; the
//! JSON content (the leading `[` and the entries) is buffered in memory and
//! written lazily — first once the buffer reaches [`DEFAULT_FLUSH_BYTES`], then
//! finally on `finish`. The on-disk byte layout is a single JSON array with one
//! entry per frame, comma-separated, no whitespace.
...
pub struct JsonTraceWriter {
    ...
    /// Whether any entry has been added yet. The leading `[` is emitted with the
    /// first entry; later entries are comma-prefixed.
    has_entries: bool,
...
impl JsonTraceWriter {
    ...
    /// Append a slice of entries. A convenience loop over [`add_entry`](Self::add_entry)
    /// that preserves the same per-entry, threshold-based flushing. Useful when
    /// the caller already has a `Vec<T>` to commit (e.g. a draining heartbeat).
    #[allow(dead_code)]
    ...
    /// Open the array on the first entry (push `[`) or separate it from the
    /// previous one (push `,`).
    ...
    /// Flush the buffer to disk if it has grown past the flush threshold.
    fn flush_if_full(&mut self) -> Result<(), JsonTraceError> {
