...
impl LossyVideoCodec {
    ...
    /// always keeps the default lossless archive. Kept pure (the config string
    /// is passed in, not read here) so the encoder path and the registration
    /// coordinator resolve identically, and the RGB-only gate is unit-testable
