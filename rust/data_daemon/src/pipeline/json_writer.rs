...
//! daemon's SQLite write-behind ([`crate::state::trace_event_database_writer`]); a single
        ...
        match msg {
            ...
            JsonWriteMsg::Finish { trace_id, ack } => {
                let result = if let Some(error) = errored.remove(&trace_id) {
                    ...
                    // No writer for this trace_id; nothing to finalise.
                    Ok(0)
                };
...
fn append_entry(
    ...
    match serde_json::from_slice::<serde::de::IgnoredAny>(payload) {
        ...
        Err(_) => {
            // A non-JSON payload should never reach here (the SDK always ships
            // JSON), so this is a defensive fallback. Surface it rather than
