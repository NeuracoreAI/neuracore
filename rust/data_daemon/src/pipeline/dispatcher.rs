...
pub enum RecordingCommand {
    ...
    /// The named recording was discarded and all local upload work must stop.
    Discard {
        cloud_recording_id: String,
        /// When the daemon learned of the discard (Unix nanoseconds).
        observed_at_ns: i64,
    },
}
...
impl Dispatcher {
    ...
    async fn handle_recording_command(&mut self, command: RecordingCommand, recv_at: Instant) {
        match command {
            ...
            RecordingCommand::Discard {
                ...
                self.handle_discard(&cloud_recording_id, observed_at_ns)
                    .await;
            }
        }
    }
    ...
    async fn handle_discard(&mut self, recording_id: &str, timestamp_ns: i64) {
        let announced = self
            ...
            .iter()
            .find(|entry| entry.value().recording_id.as_deref() == Some(recording_id))
            .map(|entry| (entry.key().clone(), entry.value().recording_index));
        let recording_index = match announced {
            Some((_, Some(index))) => index,
            Some((source, None)) => {
                self.announced.0.remove(&source);
                return;
            }
            None => match self.store.recording_index_for_cloud_id(recording_id).await {
                Ok(Some(index)) => index,
                Ok(None) => {
                    tracing::debug!(
                        recording_id,
        ...
        match self.take_window(recording_index) {
            ...
            Some((source, window)) => {
                let clears_announcement =
                    self.announced.0.get(&source).is_some_and(|announced| {
                        announced.recording_index == Some(recording_index)
                    });
                if clears_announcement {
                    self.announced.0.remove(&source);
                }
                self.held.retain(|held| {
                    held.source != source || !window.contains(held.publish_timestamp_ns)
                });
                self.discard_window(window, timestamp_ns).await;
            }
    ...
    fn take_window(&mut self, recording_index: i64) -> Option<(Source, RecordingWindow)> {
    ...
    async fn discard_window(&mut self, window: RecordingWindow, timestamp_ns: i64) {
        let recording_index = window.recording_index;
