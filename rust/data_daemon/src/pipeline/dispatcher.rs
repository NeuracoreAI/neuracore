...
impl Dispatcher {
    ...
    async fn handle_start(
        ...
        source: Source,
        dataset_id: Option<String>,
        // Set when the backend already created this recording, so its cloud id
        // is adopted rather than minted by our own `/recording/start` POST.
        ...
        match cloud_recording_id {
            // Producer-started: the start notifier POSTs `/recording/start` and
            // stamps the id it gets back.
            None => {
                if let Some(bus) = self.context.event_bus.as_ref() {
                    bus.publish(DaemonEvent::RecordingStarted { recording_index });
                }
            }
            // Backend-started: the recording already exists up there, so adopt
            // its id and skip the POST. Waiters still wake, on the event that
            // means "the cloud id is known" rather than "we just minted one".
            Some(cloud_recording_id) => {
                    ...
                    .await
                {
                    tracing::warn!(
                        ...
                        recording_index,
                        cloud_recording_id,
                        "failed to adopt the backend's recording id"
                    );
                }
                if let Some(bus) = self.context.event_bus.as_ref() {
