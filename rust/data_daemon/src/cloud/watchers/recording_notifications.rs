...
//!
//! # Why the daemon reads this itself
//!
//! Producers announce the recordings they bracket themselves, and those
//! announcements are authoritative: they arrive on the producer's own publish
//! clock, in program order with its data. A recording *nobody local* brackets —
//! started or stopped from the web — has no such announcement, and relaying one
//! through a producer does not work: a producer learns over this same stream,
//! so its relay trails the event by a network hop, and an unqualified "stop
//! what's live" that lands a hop late closes whichever recording is live *then*
//! — routinely the successor of the one that ended. Reading the stream here
//! keeps the recording's identity attached all the way to the dispatcher, which
//! acts on the named recording or not at all.
//!
//! # Failure posture
//!
//! Availability, not correctness. Producer-bracketed recordings — every offline
//! recording, and every one a local `start_recording`/`stop_recording` opens and
//! closes — work identically whether or not this task is connected. A dropped
//! connection is retried with capped backoff; only web-initiated recordings
//! stall while it is down.
...
/// Backoff ceiling. The stream is a liveness input, so a long-down backend is
/// retried steadily rather than aggressively.
const RECONNECT_MAX: Duration = Duration::from_secs(30);
/// Cap on a single SSE frame. The frames carry a recording's identity and
/// little else; anything larger is a malformed stream, not a big event.
...
/// the task handle. The channel is bounded: lifecycle events are rare, and a
/// dispatcher too busy to take one should slow this task rather than let
/// notifications queue without limit.
...
async fn run(
    ...
    // and the control-plane client's per-request deadline would sever it.
    ...
    loop {
        let Some(org_id) = org_rx.borrow().clone() else {
            // No org selected yet. Wait for one rather than polling.
            tokio::select! {
                biased;
        ...
        let outcome = tokio::select! {
            biased;
            _ = shutdown_rx.recv() => return,
            // A change of org invalidates the subscription; reconnect to the
            // new one rather than streaming another org's recordings.
            changed = org_rx.changed() => {
                if changed.is_err() {
                    return;
                }
        ...
        match outcome {
            ...
            // reconnect promptly rather than backing off as if it had failed.
            Ok(()) => {
...
async fn consume(
    ...
    if !status.is_success() {
        // A 401 means the cached token expired; drop it so the next attempt
        // exchanges a fresh one rather than retrying the same dead credential.
    ...
    // `chunk()` rather than `bytes_stream()`: it needs no `StreamExt` in scope,
    // so the daemon takes no futures dependency for one loop.
...
///
/// A frame ends at a blank line; within one, the `data:` field may be repeated
/// and the pieces are joined with newlines. Every other field — `event`, `id`,
/// `retry`, and `:` comments (which is what a keepalive is) — is ignored: this
/// stream's payload is entirely in `data`.
...
fn parse_notification(data: &str) -> Option<RecordingCommand> {
    ...
    match notification.kind.as_str() {
        // INIT replays the recordings already open when this daemon
        // subscribed, as a list. It is not handled here: adopting a recording
        // mid-flight would open a window whose lower bound is long past, and a
        // recording already underway has producers that bracketed it
        // themselves. Only starts seen live are adopted.
        "START" => {
        ...
        // A recording that is stopped, discarded, or expired is over either
        // way; each closes the window if it still owns one.
...
mod tests {
    ...
    fn keeps_a_partial_frame_for_the_next_chunk() {
        // Split mid-word: the frame is only complete once the second chunk
        // supplies the rest of it and the blank line that ends it.
        let mut buffer = b"data: one\n\ndata: hand".to_vec();
