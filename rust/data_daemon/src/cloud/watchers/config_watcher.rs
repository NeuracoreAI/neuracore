...
//! tick (covers CLI `profile update` and any external edit), and an explicit
//! refresh request delivered over an `mpsc` — the dispatcher sends one when it
//! receives a [`crate::Envelope::RefreshConfig`] command and awaits the paired
//! `oneshot` ack, so a `set_video_encoding_options → start_recording` sequence
//! from the SDK observes the new codec before the recording's traces resolve it.
