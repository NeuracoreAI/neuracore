...
//!
//! Because registration and the on-disk write can finish in either order,
//! `ReadyForUpload` is gated on BOTH states; [`publish_ready_traces`] owns that
//! promotion (and its write-behind-lag safety-net role). Registration failures
//! roll the status back to `Pending` so the next tick re-claims them.
...
pub fn spawn_registration(
    ...
    let join = tokio::spawn(async move {
        let mut ticker = interval(crate::intervals::REGISTRATION_POLL);
...
async fn submit_batch(
    ...
    for (recording_index, traces) in by_recording {
        ...
        // the lossy-only decision from the same codec source the encoder uses
        // (`LossyVideoCodec::for_trace`), so the file list we register matches
                ...
                for trace in &traces {
                    if let Some(uris) = registered_ids.get(&trace.trace_id) {
                        ...
                        if let Err(error) = store.update_trace(&trace.trace_id, update).await {
                            // The backend registered the trace but we couldn't
                            // persist it; leaving it in `registering` would wedge
                            // it for the session (no coordinator re-claims that
                            // state mid-session). Roll back to `pending` so the
                            // next tick re-claims and re-registers it.
                            tracing::warn!(%error, trace_id = trace.trace_id, "failed to persist registration outcome; rolling back to pending");
                            rollback_single_to_pending(store, &trace.trace_id).await;
                            continue;
                        }
                        ...
                        // staging burst error); retry under the shared budget.
                        handle_registration_setback(
                            store,
                            registration_attempts,
                            &trace.trace_id,
                            error.clone(),
                            error.as_deref().unwrap_or("backend rejected trace"),
                        )
                        .await;
                    } else {
                        // Backend returned neither a registered nor a failed
                        // entry for this trace; retry under the same bounded
                        // budget so a persistently-omitted trace can't loop
                        // forever.
                        handle_registration_setback(
                            store,
                            registration_attempts,
                            &trace.trace_id,
                            Some("backend returned no registration outcome".to_string()),
                            "backend silently dropped trace",
                        )
                        .await;
                    }
                }
...
/// Apply bounded-retry accounting to a trace the backend did not register —
/// either an explicit rejection or a silent omission. Rolls the trace back to
/// `pending` for another attempt, or terminally marks it `failed` once
/// [`MAX_REGISTRATION_ATTEMPTS`] is reached, so a persistently-unregisterable
/// trace can't re-claim and re-POST forever. `error_message` is the reason
/// persisted on terminal failure; `reason` is the human-readable log context.
async fn handle_registration_setback(
    store: &Arc<SqliteStateStore>,
    registration_attempts: &mut HashMap<String, u32>,
    trace_id: &str,
    ...
    *attempts += 1;
    ...
    tracing::warn!(
        ...
        reason,
        attempts = *attempts,
        "trace registration setback after retry budget exhausted; marking failed"
    );
    registration_attempts.remove(trace_id);
    let update = TraceUpdate {
        registration_status: Some(TraceRegistrationStatus::Failed),
        error_message: Some(error_message),
        ..TraceUpdate::default()
    };
    // If persisting the `failed` status itself fails, the trace would otherwise
    // sit in `registering` forever (no coordinator re-claims that state
    // mid-session), so roll it back to `pending` for the next tick.
    if let Err(persist_error) = store.update_trace(trace_id, update).await {
        tracing::warn!(%persist_error, trace_id, "failed to persist registration failure; rolling back to pending");
        rollback_single_to_pending(store, trace_id).await;
    }
}
