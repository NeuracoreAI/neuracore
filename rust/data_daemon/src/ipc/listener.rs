...
pub async fn run(
    ...
    loop {
        ...
        // The SDK's own recording state is read from here rather than from its
        // own subscription to the backend's notification stream, so this is the
        // answer every process on this host acts on.
        serve_recording_state_queries(transport.recording_state_server(), &store).await;
...
/// Drain every pending recording-state query, answering each from the daemon's
/// own store.
///
/// The daemon holds the only subscription to the backend's org-wide
/// notification stream, so a recording started or stopped from the web is
/// visible here and nowhere else on this host. SDK processes poll this to learn
/// their own recording state — including the edge that tells a producer to seal
/// its tail chunks — so an unanswered query stalls that, but cannot corrupt it:
/// a store error replies "not recording" only when the store genuinely says so,
/// and otherwise the request is dropped for the caller to retry.
///
...
async fn serve_recording_state_queries(
    ...
    loop {
        ...
        let recording = match store
            .live_recording_for_source(&query.robot_id, query.robot_instance)
            .await
        {
            Ok(recording) => recording,
            Err(error) => {
                // Drop the request rather than answering "not recording": the
                // caller reads that as a recording having ended and would drain
                // its streams on what is only a failed read.
                tracing::warn!(%error, robot_id = query.robot_id, "recording-state lookup failed");
                continue;
            }
        };

        let reply = RecordingStateReply { recording };
        match reply.encode() {
            Ok(bytes) => match active.loan_slice_uninit(bytes.len()) {
                Ok(response) => {
                    let response = response.write_from_slice(&bytes);
