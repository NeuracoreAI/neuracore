...
//!   and opens the catch-all command-stream service defined in
//!   [`data_daemon_ipc::service_name`].
//! - [`listener`]: a tokio task that polls the subscriber on a short interval
//!   and forwards decoded [`Envelope`](data_daemon_ipc::Envelope)s to the
//!   per-trace dispatcher via an `mpsc::Sender`.
//!
