...
pub mod service_name {
    ...
    /// has finished: no envelope announces "nothing more is coming", so a
    /// stopped window that is owed a flush marker no producer will send waits
    /// out this bound (plus the sweep cadence and the holdback) and settles on
    /// the source's silence instead. Diverging copies would have the daemon
    /// either drop tail chunks or hold every web-stopped recording open.
    ...
    /// nothing local bracketed — one started or stopped from the web. Rather
    /// than every SDK process holding its own subscription to the same stream
    /// and deriving state from an event order it cannot trust, each asks the
    /// daemon here for the answer it already holds. Distinct from
    /// [`RECORDING_IDS`], which resolves the cloud id of *one named* recording:
    /// this asks which recording, if any, is live right now.
    ///
    ...
    /// [`RECORDING_ID_MAX_PAYLOAD_BYTES`]: a source key one way, a recording
    /// index + optional cloud id + timestamp back.
...
pub struct LiveRecording {
    /// The daemon's local primary key. Unlike [`Self::recording_id`] it exists
    /// from the moment the recording does, so it — not the cloud id — is what
    /// tells two consecutive recordings of one source apart.
    pub recording_index: i64,
    /// The cloud handle, once `/recording/start` has been notified. `None`
    /// while the recording is still local-only (offline, or the POST is in
    /// flight).
    pub recording_id: Option<String>,
...
///
/// `recording` is `None` when the source has no open recording — it never
/// started one, the last one was stopped or cancelled, or no daemon holds any
/// record of this source at all. The three are deliberately not distinguished:
/// the caller acts on "recording" versus "not recording".
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
