...
pub(crate) fn publisher_tx() -> Sender<PublishMsg> {
    ...
    let (tx, healthy) = publisher_tx_global();
    // Only cache a channel whose receiver is alive. On a spawn failure the
    // receiver was dropped, so caching the dead `Sender` would make every later
    // `log_*` on this thread fail silently with no re-spawn; skip the cache so
    // the slow path re-attempts the spawn on the next call.
    if healthy {
...
/// return its channel. Keyed by `owner_pid` so a post-fork child re-spawns. The
/// returned flag is `false` when the spawn failed (the channel's receiver is
/// gone), so the caller knows not to cache it.
fn publisher_tx_global() -> (Sender<PublishMsg>, bool) {
...
pub(crate) fn with_producer<R>(
    ...
    PRODUCER.with(|cell| {
        // Single `borrow_mut` for the whole operation: lazily build the
        // producer state on first use, then run the operation against it.
