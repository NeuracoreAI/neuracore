...
/// it did not itself start or stop — one driven from the web.
///
/// Returns `None` when nothing answered within `timeout_s`, which the caller
/// must read as "unknown", never as "not recording": a stalled reply would
/// otherwise end every recording locally. An answer is a dict whose
/// `"recording"` is `None` (no open recording) or the recording's
/// `recording_index` / `recording_id` / `start_timestamp_ns`.
///
