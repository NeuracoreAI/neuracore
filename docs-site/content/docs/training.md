...
{{< callout type="info" >}}
Passing `run_name` is optional. If you don't pass the `run_name`, the system will generate a random name together with the timestamp. To better track the training experiment, we highly recommend you to pass a `run_name`.
{{< /callout >}}
...
python -m neuracore.ml.train algorithm=pi0 dataset_name="my_multimodal_dataset" \
  input_robot_data_spec='{"my_robot": {"JOINT_POSITIONS": ["joint_1", "joint_2"], "RGB_IMAGES": ["wrist_camera"], "LANGUAGE": ["task_instruction"]}}' \
