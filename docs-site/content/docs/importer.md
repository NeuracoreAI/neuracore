...
TFDS upload is currently not yet implemented.
{{< /callout >}}
...
| `index_range` | Range of indices (for array slicing) with `start` and `end` |
...
Ensure your dataset is in RLDS, LeRobot, or TFDS format.

### Create configuration file

Define data mappings in a YAML file.

### Prepare robot description

Have URDF or MJCF files ready.

### Run importer

Execute the CLI command with appropriate arguments.

### Monitor progress

Check logs for upload progress and any errors.
