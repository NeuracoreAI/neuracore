...
- **Rust daemon** — when `NCD_RUST_DAEMON` is truthy,
  the launcher `exec`s the native binary shipped by the optional
  `neuracore-data-daemon` package (Linux and Apple-Silicon macOS;
  `pip install neuracore[daemon]`) at
