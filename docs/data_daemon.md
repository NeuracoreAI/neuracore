...
| `video_codec` | Global video encoding for RGB cameras. `h264_lossless` (the default, also used when unset) keeps a lossless archive plus a small lossy preview. `h264_medium` instead uploads a single libx264 **CRF 23** video and drops the lossless archive — much smaller uploads, with that video used for training. Depth cameras always keep their lossless storage. Also settable from the SDK with [`nc.set_video_encoding_options`](#video-encoding-options). |
