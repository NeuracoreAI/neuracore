...
from neuracore_new_data_format.recording import SqliteRecording


class Dataset:
    def __init__(self, name: str):
        self.name = name
        self.recording: None | SqliteRecording = None
        self.recording_count = 0
        self.frame_index: defaultdict[str, int] = defaultdict(lambda: 0)

    def start_recording(self) -> None:
        self.recording = SqliteRecording(name=f"{self.name}-{self.recording_count}")
