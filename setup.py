...
class BinaryDistribution(Distribution):
    ...
    ``neuracore/data_daemon/`` are platform-specific. Without this hook
    setuptools tags the wheel ``py3-none-any``, so pip would happily
    install a Linux .so onto macOS. See
    ``docs/rust_data_daemon_development.md#packaging-the-wheel``.
    """
