...
class RLDSAndTFDSDatasetImporterBase(NeuracoreDatasetImporter):
    ...
    allow_nested_tensor_search = False
    ...
    def _handle_step_error(
        ...
        self._log_worker_error(
            self._worker_id or 0, item.index, f"Step {step_index}: {exc}"
        )
        return True
    ...
    def _extract_source_data(
        ...
        if (
            ...
            if tensor is None:
                raise ImportError(
                    "Could not find tensor in nested dict for "
                    f"{data_type.value}."
                    f"{item.name if hasattr(item, 'name') else 'unknown'}. "
                    f"Dict keys: {list(source_data.keys())}."
                )
            source_data = tensor
    ...
    def _find_tensor_in_nested(self, value: Any, tf_module: Any) -> Any | None:
        if isinstance(value, tf_module.Tensor) or (
            hasattr(value, "numpy") and not isinstance(value, (dict, list, str, bytes))
        ):
            return value
        if isinstance(value, dict):
            for nested in value.values():
                tensor = self._find_tensor_in_nested(nested, tf_module)
                if tensor is not None:
                    return tensor
        elif isinstance(value, list) and value:
            return self._find_tensor_in_nested(value[0], tf_module)
        return None
...
class RLDSDatasetImporter(RLDSAndTFDSDatasetImporterBase):
    ...
    def __init__(
        ...
        """Initialize the RLDS dataset importer."""
        super().__init__(
...
class TFDSDatasetImporter(RLDSAndTFDSDatasetImporterBase):
    ...
    allow_nested_tensor_search = True
