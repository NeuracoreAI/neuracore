...
class NeuracoreDatasetImporter(ABC):
        ...
        *,
        ...
        try:
            ...
            if not self.suppress_warnings:
                logger.warning("[WARNING] %s (%s): %s", data_type, item.name, w)
    ...
    def _find_log_dispatchers(
        self,
    ) -> tuple[RichHandler | None, list[logging.Handler]]:
        """Find the active RichHandler and any other handlers for log dispatch."""
        nc_handlers = [
            h
        ...
        return rich_handler, non_rich_handlers

    def _drain_log_queue(
        self,
        log_queue: mp.Queue | None,
        rich_handler: RichHandler | None,
        non_rich_handlers: list[logging.Handler],
        console: Console,
    ) -> None:
        """Flush pending worker log records to the parent-process handlers."""
        ...
        if rich_handler is not None:
            ...
            if formatted:
                console.print(RichGroup(*formatted))
    ...
    def _monitor_no_progress(
        ...
        rich_handler, non_rich_handlers = self._find_log_dispatchers()
        ...
        while True:
            try:
                ...
                pass
            self._drain_log_queue(log_queue, rich_handler, non_rich_handlers, console)
            any_alive = any(proc.is_alive() for proc in processes)
            if not any_alive:
                self._drain_log_queue(
                    log_queue, rich_handler, non_rich_handlers, console
                )
                break
            time.sleep(0.1)
    ...
    def _monitor_progress(
        ...
        if not show_progress:
            self._monitor_no_progress(
                processes, progress_queue, completed_queue, log_queue
            )
            return
        ...
        def drain_log_queue() -> None:
            self._drain_log_queue(
                log_queue, rich_handler, non_rich_handlers, progress.console
            )
