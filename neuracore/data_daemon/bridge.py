...
class LiveRecording(NamedTuple):
    ...
    Attributes:
        recording_index: The daemon's own primary key. Unlike ``recording_id``
            it exists from the moment the recording does, so it — not the cloud
            id — is what tells two consecutive recordings of one source apart.
        recording_id: The cloud handle, once ``/recording/start`` has been
            notified. ``None`` while the recording is still local-only.
        start_timestamp_ns: The recording's capture-clock start marker, the
            same value :meth:`RecordingContext.start_recording` returns.
    """
...
def query_recording_state(
    ...
    did not itself start or stop — one driven from the web.

    Returns:
        ``(answered, recording)``. ``answered`` is ``False`` when nothing
        replied in ``timeout_s`` — no daemon, or a busy one — and the caller
        must read that as *unknown*, never as "not recording": treating a
        missed reply as a stop would end every recording on a slow tick.
        ``recording`` is ``None`` when the daemon answered and the source has
        no open recording.
...
class RecordingContext:
    ...
    def start_recording(
        ...
        Returns:
            ...
            ``start_timestamp_ns``. It identifies the recording in the daemon's
            own records, so a caller can tell the recording it just started
