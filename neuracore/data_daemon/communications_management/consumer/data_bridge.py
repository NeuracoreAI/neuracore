...
from neuracore.data_daemon.communications_management.consumer.helpers import (
    str_or_none,
)
...
class DataBridge:
    ...
    def handle_message(self, message: MessageEnvelope) -> None:
        ...
        if self._closed_producers.contains(producer_id) and cmd != CommandType.OPEN_FIXED_SHARED_SLOTS:
            return

        if cmd == CommandType.OPEN_FIXED_SHARED_SLOTS and self._closed_producers.contains(producer_id):
    ...
    def _cleanup_expired_channels(self) -> None:
        ...
        for producer_id, state in self.channels.items():
            ...
            cutoff_sequence_number = self._trace_lifecycle.channel_stop_cutoff_sequence_number(
                producer_id, state
            )
            if (
