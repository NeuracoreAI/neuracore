...
class TraceLifecycleCoordinator:
    ...
    def handle_trace_end(self, channel: ChannelState, message: MessageEnvelope) -> None:
        ...
        if data_type_str:
            ...
            if registered_recording_id is not None:
                logger.warning(
                    "TRACE_END received for trace_id=%s without data_type metadata; "
                    "dropping unfinalizable trace for recording_id=%s",
                    trace_id,
                    recording_id,
                )
                self._trace_metadata.pop(str(trace_id))
                self._pending_trace_ends.pop(str(trace_id), None)
                self._final_chunk_enqueued_traces.discard(str(trace_id))
                self._trace_recordings.remove_trace(str(recording_id), str(trace_id))
                return
            logger.warning(
    ...
    def handle_recording_stopped(self, message: MessageEnvelope) -> None:
        ...
        if isinstance(producer_stop_sequence_numbers_raw, dict):
            for producer_id, sequence_number in producer_stop_sequence_numbers_raw.items():
                try:
