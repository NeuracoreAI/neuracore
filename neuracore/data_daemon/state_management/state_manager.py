...
class StateManager:
    ...
    def __init__(self, store: StateStore) -> None:
        ...
        # From RDEM
        emitter.on(Emitter.TRACE_WRITTEN, self._handle_trace_written)
        emitter.on(Emitter.START_TRACE, self.create_trace)

        # From uploader
        emitter.on(Emitter.UPLOAD_COMPLETE, self.handle_upload_complete)
        emitter.on(Emitter.UPLOADED_BYTES, self.update_bytes_uploaded)
        emitter.on(Emitter.UPLOAD_FAILED, self.handle_upload_failed)

        # From the data bridge
        emitter.on(Emitter.STOP_RECORDING, self.handle_stop_recording)
    ...
    async def create_trace(
        ...
        *,
        ...
        await loop.run_in_executor(
            None,
            lambda: self._store.create_trace(
                trace_id=trace_id,
    ...
    async def handle_is_connected(self, is_connected: bool) -> None:
        ...
        traces = await loop.run_in_executor(None, self._store.find_ready_traces)
        traces.sort(
            key=lambda trace: (
        ...
        unreported_traces = await loop.run_in_executor(
            None, self._store.find_unreported_traces
        )
    ...
    async def handle_upload_complete(self, trace_id: str, path: str) -> None:
        ...
        # Delete db entry
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, self._store.delete_trace, trace_id)
    ...
    async def _handle_trace_written(
        ...
        # Update db
        await loop.run_in_executor(
            None, self._store.mark_trace_as_written, trace_id, bytes_written
        )

        trace_record = await loop.run_in_executor(None, self._store.get_trace, trace_id)
        ...
        traces = await loop.run_in_executor(
            None, self._store.find_traces_by_recording_id, recording_id
        )
    ...
    async def _emit_progress_report(
        ...
        emitter.emit(Emitter.PROGRESS_REPORT, start_time, end_time, traces)
    ...
    async def mark_progress_as_reported(self, recording_id: str) -> None:
        ...
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None, self._store.mark_recording_reported, recording_id
        )
    ...
    async def update_bytes_uploaded(self, trace_id: str, bytes_uploaded: int) -> None:
        ...
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None, self._store.update_bytes_uploaded, trace_id, bytes_uploaded
        )
    ...
    async def claim_ready_traces(self, limit: int = 50) -> list[Mapping[str, Any]]:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self._store.claim_ready_traces, limit)
