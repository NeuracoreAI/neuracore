...
def _send_stop_recording_ack(
    ack_endpoint: str, recording_id: str, stopped_at: float
) -> None:
    """Send a one-shot acknowledgement that stop ownership is durable."""
    context = zmq.Context()
    socket = context.socket(zmq.PUSH)
    socket.setsockopt(zmq.IMMEDIATE, 1)
    socket.setsockopt(zmq.SNDTIMEO, 1000)
    socket.setsockopt(zmq.LINGER, 1000)
    socket.connect(ack_endpoint)
    try:
        socket.send(
            json.dumps({
...
def _to_utc_timestamp(value: datetime) -> float:
    """Return a Unix timestamp from a persisted UTC datetime."""
...
class StateManager:
    ...
    async def handle_stop_recording_requested(
        self,
        recording_id: str,
        ...
        if ack_endpoint:
            stopped_at = await self._store.get_recording_stopped_at(recording_id)
            if stopped_at is not None:
                await asyncio.get_running_loop().run_in_executor(
                    None,
                    _send_stop_recording_ack,
                    ack_endpoint,
                    recording_id,
                    _to_utc_timestamp(stopped_at),
                )
    ...
    async def _report_recording_stop(self, recording_id: str) -> bool:
        ...
        try:
            ...
            if stopped_at is None:
                last_error = "Missing stopped_at"
                await self._store.mark_recording_stop_report_pending(recording_id)
                return False

            url = f"{API_URL}/org/{org_id}/recording/stop"
            payload = {
                "recording_id": recording_id,
                "end_time": _to_utc_timestamp(stopped_at),
            }
