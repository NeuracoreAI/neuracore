...
class AlgorithmStorageHandler(UploadStorageMixin):
    ...
    def __init__(self, algorithm_id: str | None = None):
        ...
        if self.log_to_cloud:
            ...
            url = f"{API_URL}/org/{self.org_id}/algorithms/{self.algorithm_id}"
    ...
    def save_algorithm_validation_error(self, error_message: str) -> None:
        ...
        if self.log_to_cloud:
            ...
            response = session.post(url, headers=get_auth().get_headers(), json=json)
            if response.status_code != 200:
                logger.error(
    ...
    def download_algorithm(self, extract_dir: Path) -> None:
        ...
        """
        session = thread_local_session(retry_transient=True)
        url = f"{API_URL}/org/{self.org_id}/algorithms/download_url/{self.algorithm_id}"
        response = session.get(url, headers=get_auth().get_headers())
        if response.status_code != 200:
            raise ValueError(
    ...
    def save_algorithm_validation_check(
        ...
        url = (
            f"{API_URL}/org/{self.org_id}/algorithms/{self.algorithm_id}"
            "/update-algorithm-validation"
        )
        response = session.put(url, headers=get_auth().get_headers(), json=dict_to_save)
        if response.status_code != 200:
    ...
    def _get_upload_url(self, filepath: str, content_type: str) -> str:
        ...
        assert self.algorithm_id is not None, "Algorithm ID not provided"
        session = thread_local_session(retry_transient=True)
        url = f"{API_URL}/org/{self.org_id}/algorithms/{self.algorithm_id}/upload-url"
