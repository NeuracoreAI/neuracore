...
from ts.torch_handler.request_envelope.body import Body
...
class RobotModelHandler(BaseHandler):
    ...
    def _process_image_data(
        ...
        for i, images in enumerate(image_data):
            for j, (camera_name, camera_data) in enumerate(images.items()):
                image = self._decode_image(str(camera_data.frame))
    ...
    def preprocess(self, requests: List[Body]) -> BatchedInferenceSamples:
