...
class Groot(NeuracoreModel):
    ...
    def __init__(
        ...
        self.proprio_normalizer: MeanStdNormalizer | None = (
    ...
    def _combine_proprio(self, batch: BatchedInferenceInputs) -> torch.Tensor:
        ...
        if self.proprio_normalizer is None:
            return self._pad_state(all_proprio)  # (B, max_state_dim)
        return self._pad_state(
            self.proprio_normalizer.normalize(all_proprio)
        )  # (B, max_state_dim)
    ...
    def training_step(self, batch: BatchedTrainingSamples) -> BatchedTrainingOutputs:
        ...
        # Build targets; zero-fill types absent from this batch
        action_targets = []
        for data_type in self.ordered_output_data_types:
            ...
            if data_type not in batch.outputs:
                action_targets.append(
                    torch.zeros(
                        B, self.output_prediction_horizon, dim, device=self.device
                    )
                )
            elif data_type in [
        ...
        action_mask_expanded = torch.nn.functional.pad(
            torch.cat(
                [
                    (
                        ...
                        if data_type in batch.outputs_mask
                            ...
                            B,
                            self.output_dims[data_type][1]
                            - self.output_dims[data_type][0],
                            device=self.device,
                        )
                    )
                    for data_type in self.ordered_output_data_types
            ...
            (0, self.max_action_dim - self.actual_action_dim),
        ).unsqueeze(
            1
        )  # (B, 1, max_action_dim)
