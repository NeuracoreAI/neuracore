...
class CQN_AS(NeuracoreModel):
    ...
    def training_step(self, batch: BatchedTrainingSamples) -> BatchedTrainingOutputs:
        ...
        Args:
            batch: Training batch with inputs, targets, and outputs.custom_data
        ...
        action = batch.outputs.joint_target_positions.data
        action = action.reshape(B, -1)  # [B, T * action_dim]

        # Get rewards from custom_data (outputs)
        # Reward is 1 if episode completes successfully, else 0
        if "reward" not in batch.outputs.custom_data:
            raise ValueError(
                "CQN-AS requires 'reward' in outputs custom_data. "
                "Reward should be 1 if episode completes successfully, else 0."
            )
        reward = batch.outputs.custom_data["reward"].data  # [B, 1]
        # Sum across time dimension if reward has multiple timesteps
        if reward.dim() > 2:
            reward = reward.sum(dim=1)  # [B, 1]
        if reward.dim() == 1:
            reward = reward.unsqueeze(-1)  # Ensure [B, 1] shape

        # All samples are expert demonstrations
        demos = torch.ones(B, 1, device=action.device)

        # Create discount tensor
        # Use prediction mask to determine terminal states
        not_terminal = torch.min(batch.output_prediction_mask, dim=-1)[0][:, None]
