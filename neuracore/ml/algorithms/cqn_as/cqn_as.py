...
class CQN_AS(NeuracoreModel):
    ...
    For offline RL training with the neuracore dataset format:
    - Input: Single timestep observation s_t
    - Output: Action sequence a_t and future observations for s_{t+1}

    Optionally accepts CUSTOM_1D inputs with:
    - 'reward': per-timestep reward signal (default: 0 for all transitions)
    - 'terminal': 1 if terminal state, 0 otherwise (default: 0)

    If CUSTOM_1D data is not provided, uses default sparse reward assumption
    where all demonstrations are treated as expert data with uniform reward.
    ...
    - s_{t+1}: Next observation (from output RGB/joints at first future timestep)
    - r_t: Reward (from CUSTOM_1D if available, else 0)
    ...
    def training_step(self, batch: BatchedTrainingSamples) -> BatchedTrainingOutputs:
        ...
        # ============================================================
        # Reward and terminal flags from CUSTOM_1D (optional)
        # Look for scalar-valued CUSTOM_1D items (dim=1) for reward/terminal
        # ============================================================
        reward = torch.zeros(B, 1, device=action.device)
        ...
        if DataType.CUSTOM_1D in batch.inputs:
            ...
            # Find scalar-valued CUSTOM_1D items (likely reward/terminal)
            scalar_items = []
            for custom_data in custom_data_list:
                # Check if this is a scalar value (dim=1)
                if custom_data.data.shape[-1] == 1:
                    scalar_items.append(custom_data)

            # Use first scalar item as reward, second as terminal (if available)
            if len(scalar_items) >= 1:
                reward_data = scalar_items[0]
                reward = reward_data.data[:, -1, :]  # [B, 1]
                if reward.dim() == 1:
                    reward = reward.unsqueeze(-1)

            if len(scalar_items) >= 2:
                terminal_data = scalar_items[1]
                terminal = terminal_data.data[:, -1, :]  # [B, 1]
                if terminal.dim() == 1:
