...
class CNNMLP(NeuracoreModel):
    ...
    def _encode_rgb_data(
        ...
        feats = []
        for encoder, input in zip(encoders, batched_rgb_data, strict=True):
    ...
    def _encode_depth_data(
        ...
        feats = []
        for encoder, input in zip(encoders, batched_depth_data, strict=True):
    ...
    def _encode_point_cloud_data(
        ...
        feats = []
        for encoder, input in zip(encoders, batched_pc_data, strict=True):
    ...
    def _encode_language_data(
        ...
        input_ids = language_data.input_ids[:, -1, :]  # (B, L) - last timestep
        attention_mask = language_data.attention_mask[
            :, -1, :
        ]  # (B, L) - last timestep

        # Get token embeddings
        token_embeddings = encoder[0](input_ids)  # (B, L, embed_dim)

        # Apply attention mask to embeddings (zero out padding tokens)
        masked_embeddings = token_embeddings * attention_mask.unsqueeze(
            -1
        )  # (B, L, embed_dim)

        # Compute mean over non-padded tokens
        # Sum over sequence length and divide by number of non-padded tokens
        sum_embeddings = masked_embeddings.sum(dim=1)  # (B, embed_dim)
        num_tokens = attention_mask.sum(dim=1, keepdim=True)  # (B, 1)
        mean_embeddings = sum_embeddings / num_tokens.clamp(
            min=1
        )  # (B, embed_dim) - clamp to avoid division by zero

        # Project to final dimension
        return encoder[1](mean_embeddings)  # (B, final_dim)
