...
from .utils import PI0Config, pad_vector, resize_with_pad_torch
...
class Pi0(NeuracoreModel):
    ...
    def training_step(self, batch: BatchedTrainingSamples) -> BatchedTrainingOutputs:
        ...
        for data_type in self.output_data_types:
            ...
            # if len(action_targets) != len(expected_slices):
            #     raise ValueError(
            #         f"Output count for {data_type} does not match statistics. "
            #         f"Expected {len(expected_slices)}, got {len(action_targets)}"
            #     )

            # for tensor, slice_size in zip(action_targets, expected_slices):
