...
class RecordingStateManager(BaseSSEConsumer):
    ...
    def recording_started(
        ...
        Publishes the tracked state under :attr:`_state_lock` before any slower
        side effect. Notifications are resolved against this map on another
        thread, so an instance missing from it can have a notification for a
        different recording adopted in its place.
        ...
        """
        # Kept out of the critical section: it does filesystem work, and
        # `updated_recording_state` holds `_state_lock` across its whole body.
