...
class Robot:
    ...
    def __init__(
        ...
        # The recording *this process* opened, or None. Not a view of the
        # daemon's state: it is set by a local `start_recording` and by nothing
        # else, so it stays set if the recording is stopped elsewhere and says
        # nothing about one started elsewhere. Free to read, which is what the
        # log path needs — see `neuracore.api.logging.start_stream`.
    ...
    def start_recording(
        ...
        try:
            ...
            # Armed with no window behind it.
    ...
    def is_recording(self) -> bool:
        ...
        Returns:
            True if the robot is actively recording, False otherwise. Falls back
            to what this process started when no daemon answers.
        """
        if not self.id:
            raise RobotError("Robot not initialized. Call init() first.")
        answered, recording = recording_context.query_recording_state(
            self.id, self.instance
        )
        if not answered:
            return self._local_recording_handle is not None
    ...
    def get_current_recording_id(self) -> str | None:
        ...
        if not self.id:
            raise RobotError("Robot not initialized. Call init() first.")
        _, recording = recording_context.query_recording_state(self.id, self.instance)
    ...
    def _cleanup_daemon_recording_context(self) -> None:
        ...
        if self._daemon_recording_context is None:
            return
        try:
            # Seal this process's tail chunks on the way out: nothing else will
