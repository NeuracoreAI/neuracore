...
from neuracore.core.data.prefetch import DEFAULT_CONCURRENT_PREFETCH_REQUESTS
...
PAGE_SIZE = 30
# The backend validates `direction` against a case-sensitive enum, so this
# exact casing is the wire contract; lowercase yields HTTP 422. Kept private
...
class Dataset:
    ...
    def __init__(
        ...
        self._seen_recording_ids: set[str] = {r.id for r in self._recordings_cache}
    ...
    def __getstate__(self) -> dict:
        ...
        with self._page_lock:
            ...
            state["_seen_recording_ids"] = set(self._seen_recording_ids)
    ...
    def _request_recordings_page(
        ...
        cursor: dict | None,
        *,
        direction: Literal["BACKWARD"] | None = None,
    ) -> dict:
        ...
        Args:
            cursor: Raw recording dict to continue from, or None for the
                first page in this direction. Sent as the whole request body,
                which the backend binds as a single unwrapped cursor.
            direction: `BACKWARD` to start from the oldest recording and page
                toward newer ones. Omitted starts from the newest recording
                and pages toward older ones — the backend defaults an absent
    ...
    def _fetch_next_page(self, required_len: int | None = None) -> list[Recording]:
        ...
        with self._page_lock:
            ...
            self._check_page_progress(
                ...
                already_seen_ids=self._seen_recording_ids,
            )
            ...
            self._seen_recording_ids.update(r.id for r in wrapped)
            return wrapped
    ...
    def _reversed_recordings_generator(self) -> Generator[Recording, None, None]:
        ...
        loaded = 0
        while loaded < total:
            data = self._request_recordings_page(cursor, direction=_BACKWARD_DIRECTION)
            batch = data.get("data", [])
            if not batch:
                return
    ...
    def synchronize(
        ...
        max_prefetch_decode_workers: int = 4,
        ...
        return SynchronizedDataset(
            ...
            max_prefetch_decode_workers=max_prefetch_decode_workers,
            num_concurrent_prefetch_requests=num_concurrent_prefetch_requests,
            max_delay_s=max_delay_s,
            allow_duplicates=allow_duplicates,
            trim_start_end=trim_start_end,
            trim_no_movement_at_start_threshold=trim_no_movement_at_start_threshold,
