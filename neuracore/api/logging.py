...
def log_target_parallel_gripper_open_amount(
    name: str,
    value: float,
    robot_name: str | None = None,
    instance: int = 0,
    ...
    """Log target parallel gripper open amount data for a robot.
...
def log_target_parallel_gripper_open_amounts(
    values: dict[str, float],
    robot_name: str | None = None,
    instance: int = 0,
    timestamp: float | None = None,
    ...
    timestamp = timestamp or time.time()
    for name, value in values.items():
        log_target_parallel_gripper_open_amount(
            name=name,
            value=value,
            robot_name=robot_name,
            instance=instance,
