...
def _log_single_joint_data(
    ...
    """
    str_id = f"{data_type.name.lower()}:{name}"
    joint_stream = robot.get_data_stream(str_id)
    if joint_stream is None:
        joint_stream = JsonDataStream(data_type=data_type, data_type_name=name)
...
def _log_camera_data(
    ...
    if stream is None:
        if camera_type == DataType.RGB_IMAGES:
            stream = RGBDataStream(str_id, image.shape[1], image.shape[0])
        elif camera_type == DataType.DEPTH_IMAGES:
            stream = DepthDataStream(str_id, image.shape[1], image.shape[0])
        else:
...
def log_custom_1d(
    ...
    stream = robot.get_data_stream(str_id)
    if stream is None:
        stream = JsonDataStream(data_type=DataType.CUSTOM_1D, data_type_name=name)
        robot.add_data_stream(str_id, stream)
...
def log_pose(
    ...
    stream = robot.get_data_stream(str_id)
    if stream is None:
        stream = JsonDataStream(data_type=DataType.POSES, data_type_name=name)
        robot.add_data_stream(str_id, stream)
...
def log_end_effector_pose(
    ...
    if stream is None:
        stream = JsonDataStream(
            data_type=DataType.END_EFFECTOR_POSES, data_type_name=name
        )
        robot.add_data_stream(str_id, stream)
...
def log_parallel_gripper_open_amount(
    ...
    if stream is None:
        stream = JsonDataStream(
            data_type=DataType.PARALLEL_GRIPPER_OPEN_AMOUNTS, data_type_name=name
        )
        robot.add_data_stream(str_id, stream)
...
def log_language(
    ...
    stream = robot.get_data_stream(str_id)
    if stream is None:
        stream = JsonDataStream(data_type=DataType.LANGUAGE, data_type_name=name)
        robot.add_data_stream(str_id, stream)
    start_stream(robot, stream)
    assert isinstance(
        stream, JsonDataStream
...
def log_point_cloud(
    ...
    stream = robot.get_data_stream(str_id)
    if stream is None:
        stream = JsonDataStream(data_type=DataType.POINT_CLOUDS, data_type_name=name)
        robot.add_data_stream(str_id, stream)
    assert isinstance(
        stream, JsonDataStream
    ), "Expected stream to be instance of JSONDataStream"
