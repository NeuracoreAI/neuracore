...
def setup_logging(
    ...
    Args:
        ...
        json_format: Emit JSON lines to console (and file when log_file set).
            Defaults to NEURACORE_LOG_JSON env var, or False.
        ...
        rich_console: When True, use RichHandler for console even when
            json_format=True. The file handler still uses JSON. Useful for
            training runs where a human watches stderr but the log file is
    ...
    handlers = (
        ...
        else _create_handlers(json_format, log_file, console, rich_console)
    )
...
def _create_handlers(
    ...
    if json_format and not rich_console:
        ...
        console_handler.setFormatter(JsonLineLogFormatter())
    else:
